...
Board::Board() {
    ...
    for(int i=0; i<28; ++i){
        ...
        dynamic_cast<property*>(board[properties[i]->getposition()])->setowner(input_int);
        ...
        if(type_info == typeid(street)){
            ...
            while(input_int > 0){
                (static_cast<street*>(board[properties[i]->getposition()]))->addhouse();
                --input_int;
            }
        }
    }
...
void Board::save(const std::string& filename){
    ...
    for(int i=0; i<40; ++i){
        ...
        property* current = dynamic_cast<property*>(board[i]);
        if(current){
            output_file << current->getowner() << std::endl;
            if(typeid(*board[i]) == typeid(street)){
